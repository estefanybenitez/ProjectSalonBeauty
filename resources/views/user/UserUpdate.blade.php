...
<form method="POST" action="/user/update/{{ $user->id_user}}" class="mx-auto max-w-md">
    @csrf
    @method('PUT')

    <p class="mt-4 text-4xl text-gray-900 dark:text-white text-center"><b>Actualizar Usuario</b></p>

    <div class="mx-auto max-w-md">
        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nombre</label>
        <input id="name" type="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Nombre" @error('name') is-invalid @enderror name="name" value="{{ $user->name }}" required autocomplete="name" autofocus>
