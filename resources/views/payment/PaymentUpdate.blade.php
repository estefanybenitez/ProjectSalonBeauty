...
<form method="POST" action="/payment/update/{{ $payment->id_payment }}" class="mx-auto max-w-md">
    @csrf
    @method('PUT')

    <p class="mt-4 text-4xl text-gray-900 dark:text-white text-center"><b>Actualizar Pago</b></p>
    ...
    <div class="mt-4 mx-auto max-w-md">
        ...
        <select name="fk_user" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            @foreach ($users as $item)
                <option value="{{ $item->user_id }}" {{ $item->user_id == $payment->fk_user ? 'selected' : '' }}>{{ $item->name }}</option>
            @endforeach
        </select>
        @error('fk_user')
            <span class="invalid-feedback" role="alert">
    ...
    <div class="row mb-0">
        <div class="col-md-6 offset-md-4">
            <br>
            <button type="submit" class="text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-lg text-sm px-5 py-2 text-center me-2 mb-2">Guardar</button> 
        </div>
    </div>
</form>
